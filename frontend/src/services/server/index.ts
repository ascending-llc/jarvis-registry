...
const toggleServerStatus: (id: string, data: { enabled: boolean }, config?: AxiosRequestConfig) => Promise<void> =
  async (id, data, config) => await Request.post(API.toggleServerStatus(id), data, config);
