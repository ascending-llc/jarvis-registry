// @ts-nocheck
import { Handle, Position } from '@xyflow/react';
import '../../styles/nodes.css';

/* Compact spacing between handles *inside* the node card.
   The canvas Add-node spacing (120px) is larger intentionally — edges will
   angle slightly, which is fine and keeps the cards compact. */
const HANDLE_SPACING_PX = 36;

/* ── Shared header ──────────────────────────────────────────────────────────── */
function Header({ iconClass, iconLabel, title, dotClass }) {
  return (
    <div className='node-header'>
      <div className={`node-icon ${iconClass}`}>{iconLabel}</div>
      <div className='node-title'>{title}</div>
      <div className={`node-dot ${dotClass}`} />
    </div>
  );
}

/* ── MCP Server ─────────────────────────────────────────────────────────────── */
export function McpNode({ data, selected }) {
  return (
    <div className={`node-wrap mcp ${selected ? 'selected' : ''}`}>
      <Handle type='target' position={Position.Left} />
      <Header iconClass='mcp' iconLabel='MCP' title={data.label} dotClass='mcp' />
      <div className='node-body'>{data.description}</div>
      <Handle type='source' position={Position.Right} />
    </div>
  );
}

/* ── A2A Agent ──────────────────────────────────────────────────────────────── */
export function AgentNode({ data, selected }) {
  return (
    <div className={`node-wrap agent ${selected ? 'selected' : ''}`}>
      <Handle type='target' position={Position.Left} />
      <Header iconClass='agent' iconLabel='A2A' title={data.label} dotClass='agent' />
      <div className='node-body'>{data.description}</div>
      <Handle type='source' position={Position.Right} />
    </div>
  );
}

/* ── Approval Gate (HITL) ───────────────────────────────────────────────────── */
export function GateNode({ data, selected }) {
  return (
    <div className={`node-wrap gate ${selected ? 'selected' : ''}`}>
      <Handle type='target' position={Position.Left} />
      <Header iconClass='gate' iconLabel='⏸' title={data.label || 'Approval Gate'} dotClass='gate' />
      <div className='node-body'>{data.description || 'Pause run for human review'}</div>
      <div className='node-footer'>
        <span className='node-badge gate'>⏸ awaiting human</span>
        {data.timeout && <span className='node-hint'>{data.timeout}</span>}
      </div>
      <Handle type='source' position={Position.Right} />
    </div>
  );
}

/* ── Conditional (if / else) ────────────────────────────────────────────────── */
/* Both true and false source handles are on the right side, spread vertically  */
/* minHeight ensures both handles sit comfortably inside the node */
export function CondNode({ data, selected }) {
  return (
    <div
      className={`node-wrap cond ${selected ? 'selected' : ''}`}
      style={{ minHeight: HANDLE_SPACING_PX + 60 }}
    >
      <Handle type='target' position={Position.Left} />
      <Header iconClass='cond' iconLabel='if' title={data.label || 'Conditional'} dotClass='cond' />
      <div className='node-body'>
        <code
          style={{
            fontSize: 10,
            fontFamily: 'JetBrains Mono, monospace',
            color: 'var(--wf-blue)',
            wordBreak: 'break-all',
          }}
        >
          {data.expression || 'session_state.score > 0.8'}
        </code>
      </div>
      <div className='node-footer'>
        <span className='node-badge blue'>true →</span>
        <span className='node-badge gray'>false →</span>
      </div>
      <Handle
        type='source'
        position={Position.Right}
        id='true'
        style={{
          top: `calc(50% - ${HANDLE_SPACING_PX / 2}px)`,
          background: 'var(--wf-blue)',
          borderColor: 'var(--wf-blue)',
        }}
      />
      <Handle
        type='source'
        position={Position.Right}
        id='false'
        style={{
          top: `calc(50% + ${HANDLE_SPACING_PX / 2}px)`,
          background: 'var(--wf-text-4)',
          borderColor: 'var(--wf-border-strong)',
        }}
      />
    </div>
  );
}

/* ── Parallel (fan-out) ─────────────────────────────────────────────────────── */
/* One source handle per branch, spread vertically on the right.                 */
/* Handle offsets use the same BRANCH_SPACING_PX formula as the canvas so edges  */
/* land exactly on the Add placeholder nodes.                                    */
export function ParallelNode({ data, selected }) {
  const branches = Array.isArray(data.branches) ? data.branches : ['Branch A', 'Branch B'];
  const N = branches.length;
  const minHeight = Math.max(80, (N - 1) * HANDLE_SPACING_PX + 60);
  return (
    <div className={`node-wrap parallel ${selected ? 'selected' : ''}`} style={{ minHeight }}>
      <Handle type='target' position={Position.Left} />
      <Header iconClass='parallel' iconLabel='∥' title={data.label || 'Parallel'} dotClass='parallel' />
      <div className='node-body'>Fan-out — runs all branches concurrently</div>
      <div className='node-footer'>
        {branches.map((b, i) => (
          <span key={i} className='node-badge teal'>
            {b}
          </span>
        ))}
        <span className='node-hint'>+ add in props</span>
      </div>
      {branches.map((_, i) => (
        <Handle
          key={i}
          type='source'
          position={Position.Right}
          id={`branch-${i}`}
          style={{
            top: `calc(50% + ${(i - (N - 1) / 2) * HANDLE_SPACING_PX}px)`,
            background: 'var(--wf-teal)',
            borderColor: 'var(--wf-teal)',
          }}
        />
      ))}
    </div>
  );
}

/* ── Router (switch / case) ─────────────────────────────────────────────────── */
export function RouterNode({ data, selected }) {
  const cases = Array.isArray(data.cases) ? data.cases : ['critical', 'normal'];
  const N = cases.length;
  const minHeight = Math.max(80, (N - 1) * HANDLE_SPACING_PX + 60);
  return (
    <div className={`node-wrap router ${selected ? 'selected' : ''}`} style={{ minHeight }}>
      <Handle type='target' position={Position.Left} />
      <Header iconClass='router' iconLabel='⇄' title={data.label || 'Router'} dotClass='router' />
      <div className='node-body'>
        <span style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: 'var(--wf-text-4)' }}>
          switch {data.routeBy || 'session_state.severity'}
        </span>
      </div>
      <div className='node-footer'>
        {cases.map((c, i) => (
          <span key={i} className='node-badge pink'>
            case: {c}
          </span>
        ))}
        {data.defaultCase && <span className='node-badge gray'>default: {data.defaultCase}</span>}
      </div>
      {cases.map((_, i) => (
        <Handle
          key={i}
          type='source'
          position={Position.Right}
          id={`case-${i}`}
          style={{
            top: `calc(50% + ${(i - (N - 1) / 2) * HANDLE_SPACING_PX}px)`,
            background: 'var(--wf-pink)',
            borderColor: 'var(--wf-pink)',
          }}
        />
      ))}
    </div>
  );
}

/* ── Loop ────────────────────────────────────────────────────────────────────── */
export function LoopNode({ data, selected }) {
  return (
    <div className={`node-wrap loop ${selected ? 'selected' : ''}`}>
      <Handle type='target' position={Position.Left} />
      {/* Back-edge comes in from bottom */}
      <Handle
        type='target'
        position={Position.Bottom}
        id='back'
        style={{ background: 'var(--wf-orange)', borderColor: 'var(--wf-orange)' }}
      />
      <Header iconClass='loop' iconLabel='↻' title={data.label || 'Loop'} dotClass='loop' />
      <div className='node-body'>
        Max <strong style={{ color: 'var(--wf-orange)' }}>{data.maxIterations || 5}</strong> iterations
        {data.exitCondition && (
          <div style={{ marginTop: 3 }}>
            <code style={{ fontSize: 10, fontFamily: 'JetBrains Mono, monospace', color: 'var(--wf-text-4)' }}>
              exit: {data.exitCondition}
            </code>
          </div>
        )}
      </div>
      <div className='node-footer'>
        <span className='node-badge orange'>↻ body →</span>
        <span className='node-badge gray'>exit ↓</span>
      </div>
      <Handle
        type='source'
        position={Position.Right}
        id='body'
        style={{ background: 'var(--wf-orange)', borderColor: 'var(--wf-orange)' }}
      />
      <Handle
        type='source'
        position={Position.Bottom}
        id='exit'
        style={{ background: 'var(--wf-text-4)', borderColor: 'var(--wf-border-strong)' }}
      />
    </div>
  );
}

/* ── Agent Pool ──────────────────────────────────────────────────────────────── */
export function PoolNode({ data, selected }) {
  const agents = Array.isArray(data.agents) ? data.agents : [];
  const remaining = 5 - agents.length;
  return (
    <div className={`node-wrap pool ${selected ? 'selected' : ''}`}>
      <Handle type='target' position={Position.Left} />
      <Header iconClass='pool' iconLabel='◈' title={data.label || 'Agent Pool'} dotClass='pool' />
      <div className='node-body'>LLM selects best-fit agent at runtime</div>
      <div className='node-footer'>
        {agents.map((a, i) => (
          <span key={i} className='node-badge purple'>
            {a}
          </span>
        ))}
        {remaining > 0 && <span className='node-hint'>+ add up to {remaining}</span>}
      </div>
      <Handle type='source' position={Position.Right} />
    </div>
  );
}

/* ── Add-node placeholder ────────────────────────────────────────────────────── */
export function AddNode({ data }) {
  return (
    <>
      <Handle type='target' position={Position.Left} />
      <div className='add-node' onClick={data.onAdd}>
        <div className='add-plus'>+</div>
        <div className='add-label'>Add next node</div>
      </div>
    </>
  );
}

/* ── nodeTypes map ───────────────────────────────────────────────────────────── */
export const nodeTypes = {
  mcp: McpNode,
  agent: AgentNode,
  gate: GateNode,
  cond: CondNode,
  parallel: ParallelNode,
  router: RouterNode,
  loop: LoopNode,
  pool: PoolNode,
  add: AddNode,
};
