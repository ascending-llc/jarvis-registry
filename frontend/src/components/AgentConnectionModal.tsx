...
const buildCurlCommand = (agentUrl: string): string => `curl -sS -X POST "${agentUrl}" \\
    ...
    \\"params\\": {
      \\"message\\": {
        ...
        \\"parts\\": [
          {
            \\"kind\\": \\"text\\",
            \\"text\\": \\"Describe what you can help me with.\\"
          }
        ]
      }
    }
...
const AgentConnectionModal: React.FC<AgentConnectionModalProps> = ({
  ...
  const curlCommand = useMemo(() => (agentUrl ? buildCurlCommand(agentUrl) : ''), [agentUrl]);
  ...
  return (
    <Dialog open={isOpen} onClose={onClose} className='relative z-50'>
      ...
      <div className='fixed inset-0 flex items-center justify-center overflow-y-auto px-4 py-6'>
        <Dialog.Panel className='relative max-h-[90vh] w-full max-w-[760px] overflow-y-auto rounded-xl border border-[color:var(--jarvis-border)] bg-[var(--jarvis-bg)] p-7 text-[var(--jarvis-text)] shadow-2xl'>
