...
interface ResourceConsentPageProps {
  getConsentContext: (nonce: string) => Promise<ConsentContext>;
  approveConsent: (nonce: string) => Promise<ConsentDecisionResponse>;
  denyConsent: (nonce: string) => Promise<ConsentDecisionResponse>;
  getDescription: (context: ConsentContext) => string;
}
...
const ResourceConsentPage: React.FC<ResourceConsentPageProps> = ({
  getConsentContext,
  approveConsent,
  denyConsent,
  getDescription,
  ...
  const handleApprove = useCallback(async () => {
    setApproving(true);
    try {
      const result = await approveConsent(nonce);
  ...
  }, [approveConsent, nonce]);

  const handleDeny = useCallback(async () => {
    setDenying(true);
    try {
      const result = await denyConsent(nonce);
      setClientBranding(result.client_branding);
      setDenied(true);
    } catch (err) {
...
export default ResourceConsentPage;
