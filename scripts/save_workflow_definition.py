...
async def _active_executor_keys() -> tuple[list[str], list[str], list[A2AAgent]]:
    ...
    a2a_agents = await A2AAgent.find({"config.enabled": True}).to_list()
    mcp_keys = sorted(server.serverName for server in mcp_servers)
    a2a_keys = sorted(agent.path for agent in a2a_agents)
...
async def _print_node_agent_details(nodes: list[WorkflowNode]) -> None:
    ...
    for node in nodes:
        if node.executor_key:
            referenced_paths.add(node.executor_key.strip("/"))
        if node.a2a_pool:
            for key in node.a2a_pool:
                referenced_paths.add(key.strip("/"))

    if not referenced_paths:
        return

    agents = await A2AAgent.find({"path": {"$in": sorted(referenced_paths)}, "config.enabled": True}).to_list()
    if not agents:
        return
