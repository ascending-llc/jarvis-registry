    ...
    fake_workflow = WorkflowDefinition.model_construct(
        ...
        nodes=[
            WorkflowNode(
                ...
                true_steps=[WorkflowNode(name="C", node_type="step", executor_key="tool-c", step_objective="run C")],
                false_steps=[WorkflowNode(name="D", node_type="step", executor_key="tool-d", step_objective="run D")],
            )
        ],
...
async def test_trigger_run_forwards_requested_version(monkeypatch):
    ...
    collect_pending.assert_awaited_once()
