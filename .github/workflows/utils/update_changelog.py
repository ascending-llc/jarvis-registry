...
# ── Read env vars ──────────────────────────────────────────────────────────────
tag          = os.environ["RELEASE_TAG"]               # e.g. v0.3.2
name         = os.environ.get("RELEASE_NAME") or tag   # e.g. "Jarvis Registry v0.3.2"
...
slug = tag  # e.g. v0.3.2  →  changelog/v0.3.2.md
...
# Body: convert GitHub-style "## What's Changed" etc. to MkDocs-friendly md.
# We keep it verbatim but add a back-link at the top.
page_content = (
    frontmatter
    + f"[← Back to changelog](index.md)\n\n"
...
new_row = (
    f"| [{tag}]({slug}.md) | {date_str} | {tag_label} | "
    f"{description} |\n"
)
...
# Load as plain text so we preserve comments and formatting
mkdocs_text = mkdocs_path.read_text(encoding="utf-8")

# The nav entry we want to insert:  "  - vX.Y.Z: changelog/vX.Y.Z.md\n"
new_nav_entry = f"  - {tag}: changelog/{slug}.md\n"

# We look for the Changelog section header and insert right after the
# Overview line so entries are newest-first.
# Pattern to match:
#   - Changelog:
#     - Overview: changelog/index.md
#     - v0.3.1: changelog/v0.3.1.md   ← insert new entry here (above this)
...
if changelog_section_re.search(mkdocs_text):
    ...
    # Changelog section doesn't exist yet — append it before the "- Project:" block
    project_re = re.compile(r"(^- Project:)", re.MULTILINE)
    changelog_block = (
        f"- Changelog:\n"
        f"  - Overview: changelog/index.md\n"
        f"  {new_nav_entry}"
        f"\n"
    )
    if project_re.search(mkdocs_text):
        mkdocs_text = project_re.sub(changelog_block + r"\1", mkdocs_text, count=1)
        print(f"✅  Bootstrapped Changelog nav section in mkdocs.yml")
    else:
        mkdocs_text = mkdocs_text.rstrip("\n") + "\n" + changelog_block
        print(f"✅  Appended Changelog nav section to mkdocs.yml")
