...
_SAFE_SERVER_NAME_RE = re.compile(r"^[a-zA-Z0-9_.-]+\Z")
...
def normalize_server_name(server_name: str) -> str:
    """Port of librechat-data-provider's normalizeServerName (packages/data-provider/src/config.ts:3532-3549).

    Must stay byte-for-byte identical to the TS implementation: Chat's Mongoose schema and
    Registry's Beanie model share the same `mcpservers` collection and the same unique index
    on this field. Any future change to the TS function must be ported here too, since there
    is no shared implementation between the two languages.
    """
    if _SAFE_SERVER_NAME_RE.match(server_name):
        return server_name

    normalized = _UNSAFE_CHAR_RE.sub("_", server_name).strip("_")
    if normalized:
        return normalized

    # All characters were stripped; hash the original so the name stays unique/deterministic.
    # Iterate UTF-16 code units (like JS's String.charCodeAt), not Python's Unicode code
    # points, so a serverName containing astral characters (e.g. emoji) hashes identically
    # to the TS implementation.
    utf16_bytes = server_name.encode("utf-16-be", "surrogatepass")
    code_units = struct.unpack(f">{len(utf16_bytes) // 2}H", utf16_bytes)
    hash_value = 0
    for code_unit in code_units:
        hash_value = (hash_value << 5) - hash_value + code_unit
        hash_value &= 0xFFFFFFFF
    if hash_value >= 0x80000000:
        hash_value -= 0x100000000
    return f"server_{abs(hash_value)}"
...
class ExtendedMCPServer(MCPServer):
    ...
    # Override: the generated base class declares this as required (str, no default) once
    # jarvis-api#407 lands, matching Mongoose's `required: true`. Most existing documents in
    # the shared `mcpservers` collection predate this field. Keeping it Optional here decouples
    # deploying this code from finishing the backfill (scripts/backfill_mcp_server_normalized_names.py) —
    # tighten back to `str` (required) only after that script's --dry-run reports zero missing/
    # stale documents across every environment.
    normalizedServerName: str | None = None
    ...
    def from_server_info(cls, server_info: dict[str, Any], is_enabled: bool = False) -> "ExtendedMCPServer":
        ...
        return cls(
            ...
            normalizedServerName=normalize_server_name(server_name),
            path=path,
            config=config,
